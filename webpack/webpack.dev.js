...
module.exports = {
  ...
  devServer: {
    contentBase: path.resolve(__dirname, "../dist"), //这句话"一定要写"，令webpack-dev-server以此做为静态文件服务器的目录，这样就能够"同时使用"内存中的动态文件和dist目录中的真实文件了
    port: 3000,
    open: true,
    hot: true, //开启hmr热替换(👍)
  ...
  output: {
    ...
    chunkFilename: "./js/chunk.[name].js", // chunkhash是文件级别的，一个文件修改，自身和与它相关文件的hash重新编译（注：另一个是contenthash,是内容级别的,比前两者的影响范围更细更小,只会使自身的hash重新编译，但需要相应插件支持，并且需要在相应插件中设置）
  },
  ...
  plugins: [
    ...
    new Webpack.DllReferencePlugin({
      manifest: path.resolve(
        __dirname,
        "../dist/js/dll/vendor.dll.manifest.json"
      ),
    }),
